function [ADC_DE,ADC_DE_allcmpts,elapsed_time] ...
    = HADC_JRL(experiment,mymesh,DIFF_cmpts,IC_cmpts)
...
for icmpt = 1:mymesh.Ncmpt
    ...
    if sum(size(neumann))>0
        if (1 == 1)
            [FacA,FacC,FacN,EleC] = ...
                get_surfacenormal_mesh(mymesh.Pts_cmpt_reorder{icmpt},mymesh.Ele_cmpt_reorder{icmpt},neumann');
            coordNORMALSnew = FacN';
        else
