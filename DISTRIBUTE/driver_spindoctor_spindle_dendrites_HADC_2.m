...
fname_params_cells = 'params_cells_spindle_dendrites_HADC_2.in';
fname_params_simul_domain  = 'params_simul_domain.in';
fname_params_simul_experi = 'params_simul_experi_spindle_dendrites_HADC_2.in';
...
if params_cells.cell_shape == 3
    mymesh = read_mesh_2(fname_tetgen_femesh, DIFF_cmpts(1));
else 
...
if (~isempty(mymesh))
    ...
    if (experi_common.ngdir_total == 1)
        ...
        if (~isempty(experi_btpde))
            ...
            for i=2:3
                h = PLOT_3Dsignal(grad_dir, real_signal_allcompts_normalized(:,i),experi_btpde.bvalues(i));    
            end
        end
        %HADC
        if (~isempty(experi_hadc))
            ...
            % save('dendrites_HADC_2.mat')
            
            % load('dendrites_HADC_2.mat')
            h = PLOT_HARDI(points,ADC_HADC_allcmpts_alldir);
        end
    end
