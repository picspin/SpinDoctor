...
load workspace.mat
mm = createpde();
geometryFromMesh(mm,X06b_spindle8aACC_nodes',X06b_spindle8aACC_elements');
pdeplot3D(mm);
% mymesh=mm;
mymesh.Pts_cmpt_reorder{1,1} = X06b_spindle8aACC_nodes';
mymesh.Ele_cmpt_reorder{1,1} = X06b_spindle8aACC_elements';
mymesh.Pts_ind{1,1} = [1:size(X06b_spindle8aACC_nodes,1)]';
%%
if (~isempty(mymesh))
