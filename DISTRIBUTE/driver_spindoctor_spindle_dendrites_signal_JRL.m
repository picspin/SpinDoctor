...
fname_params_simul_experi = 'params_simul_experi_spindle_dendrites_signal.in';
...
%                 = BTPDE_signal(experi_btpde,mymesh,DIFF_cmpts,kappa_bdys,IC_cmpts);
            ...
            for iexperi = 1:nexperi
                for ib = 2:nb
                    ...
                    PLOT_HARDI(points_gdir,squeeze(real(SIG_BTPDE_allcmpts_alldir(:,iexperi,ib)))/S0);                   
                end
            end
            
%             % load('dendrites_signal20_4000.mat')
...
%             end           
%             title_str = ['BTPDE'];
%             PLOT_HARDI_PT(grad_dir,real_signal_allcompts_normalized(:,i),title_str);
        end
        %HADC
        if (~isempty(experi_hadc))
